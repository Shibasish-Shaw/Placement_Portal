...
<body>
  <div class="sidebar">
   ...
   <section class="home-section">
    <div class="text">Job details of <?php echo $_SESSION['user_name']; ?><hr>

<?php

//need to work for show all offer deatils
    try {
     ...
     //echo $cmp_id;
      // print_r($_SESSION);
       ...
       $stmt=$conn->query("SELECT * FROM setplacement.job j WHERE j.cmp_id=$cmp_id ");
       if($stmt ->rowcount() > 0){
        ...
        // rec_key to get rec_ deatils
          try {
            $stmt2=$conn->query("SELECT * FROM SetPlacement.recomendation r WHERE r.rec_id=\"$rec_id\"  ");
            if($stmt2->rowcount() > 0){
              while ($set4=$stmt2->fetch()) {
          ...
          <table>
            <tr>
              Profile name : <?php echo $profile; ?>
            </tr>
          <?php

              //get offer details for comany

          try{
            $stmt1=$conn->query("SELECT * FROM setplacement.offers o WHERE o.job_id=$job_id");
            ?>
              <tr>
                <th>Student name </th>
                <th>Roll No</th>
            ...
            // rec_key to get rec_ deatils
          try {
              $stmt3=$conn->query("SELECT * FROM SetPlacement.student s WHERE s.rollNo = $rollNo ");
            if($stmt3->rowcount() > 0){
              while ($set2=$stmt3->fetch()) {
                    //  print_r($set2);
              ?>
              <tr>
                <td><?php echo $set2['name'] ;?></td>
                <td><?php echo $set2['rollNo'];  ?></td>
                <td><?php echo $set2['email'];?></td>
                <td><?php echo $set2['programme']; ?></td>
                <td><?php  echo $set2['depertment'];?></td>
                <td><?php  echo $set2['gender'];?></td>
                <td><?php  echo $set1['aecepted'];?></td>
                <td><?php  echo $set1['offerlastdate'];?></td>
              </tr>
              <?php
                    }
                  }                    
